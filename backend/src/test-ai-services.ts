...
// import meetingService from './services/meetingService'; // Removed - using Transcript model directly
...
async function testAIServices() {
	...
	try {
		...
		// COMMENTED OUT - Meeting service removed, using Transcript model directly
		/*
		console.log("=".repeat(60))
		...
		const processedMeeting = await meetingService.processMeeting(
		...
		console.log(
			`   Summary length: ${processedMeeting.summary?.length || 0} characters`
		)
		console.log(
			`   Action items created: ${processedMeeting.actionItems.length}`
		)

		// Fetch action items
		console.log("\n📋 Action Items from Database:")
		const items = await actionItemService.getActionItemsByMeeting(
			meeting._id.toString()
		)
		items.forEach((item, index) => {
			console.log(
				`   ${index + 1}. [${item.priority}] ${item.text} (${item.status})`
			)
		})
