...
class ActionItemService {
	...
	async getActionItemById(id: string): Promise<IActionItem | null> {
		try {
			const actionItem = await ActionItem.findById(id).populate(
				"meetingId",
				"title date"
			)
			return actionItem
	...
	async getActionItemsByMeeting(meetingId: string): Promise<IActionItem[]> {
		try {
			const actionItems = await ActionItem.find({ meetingId }).sort({
				priority: 1,
				createdAt: -1,
			})
			return actionItems
	...
	async updateActionItem(
		...
		try {
			const actionItem = await ActionItem.findByIdAndUpdate(id, updates, {
				new: true,
			})
