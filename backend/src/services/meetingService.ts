...
class MeetingService {
	...
	async getMeetings(filters: MeetingFilters = {}): Promise<any[]> {
		try {
			...
			const meetings = await Meeting.find(query).sort({ date: -1 }).lean()
	...
	async getMeetingById(id: string): Promise<any> {
		try {
			const meeting = await Meeting.findById(id).populate("actionItems").lean()
	...
	async processMeeting(id: string): Promise<IMeeting> {
		try {
			...
			if (!transcriptText && meeting.transcriptUrl) {
				...
				transcriptText = await nylasService.downloadTextFile(
			...
			// 5. Extract participants with AI
			const participants = await openaiService.extractParticipants(
				transcriptText
			)
			meeting.participants = participants
			await meeting.save()

			// 6. Extract action items with AI
			const extractedItems: ExtractedActionItem[] =
				await openaiService.extractActionItems(transcriptText)
			...
			if (extractedItems.length > 0) {
				...
				const createdActionItems = await actionItemService.createActionItems(
					actionItemsData
				)
